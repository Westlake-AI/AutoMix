...
torch.backends.cudnn.benchmark = True  # disable for stable reproduction
...
parser.add_argument('--crop_pct', default=0.85, type=float,
    ...
    try:
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            train_metrics, all_entropy = train_one_epoch(
                epoch, model, loader_train, optimizer, train_loss_fn, args, _logger,
            ...
            eval_metrics = evaluate(model, loader_eval, validate_loss_fn, args, _logger, amp_autocast=amp_autocast)

            if model_ema is not None and not args.model_ema_force_cpu:
                if args.distributed and args.dist_bn in ('broadcast', 'reduce'):
                    distribute_bn(model_ema, args.world_size, args.dist_bn == 'reduce')
                ema_eval_metrics = evaluate(
                    model_ema.module, loader_eval, validate_loss_fn, args, _logger, amp_autocast=amp_autocast, log_suffix=' (EMA)')
